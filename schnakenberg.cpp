...
void savePngs (const std::string& logpath, const std::string& name,
               unsigned int frameN, mplot::Visual<>& v)
{
...
int main (int argc, char **argv)
{
    ...
    /*
     * Set up mplot::Config (JSON reader/writer) for reading the parameters
     */
    ...
    if (logpath == "fromfilename") {
        ...
        std::vector<std::string> pth = mplot::tools::stringToVector (justfile, "/");
        justfile = pth.back();
        mplot::tools::searchReplace (".json", "", justfile);
    ...
    // Set up the mplot::Visual object which provides the visualization scene (and
    // a GLFW window to show it in)
    mplot::Visual v1 (win_width, win_height, "Schnakenberg RD");
     ...
     * failures.
     */
    if (mplot::tools::dirExists (logpath) == false) {
        mplot::tools::createDir (logpath);
        if (mplot::tools::dirExists (logpath) == false) {
        ...
        if (overwrite_logs == false
            && (mplot::tools::fileExists (logpath + "/params.json") == true
                || mplot::tools::fileExists (logpath + "/positions.h5") == true)) {
    ...
    mplot::ColourMapType cmt = mplot::ColourMap<FLT>::strToColourMapType (conf.getString ("colourmap", "Jet"));

    // Create a new HexGridVisual then set its parameters (zScale, colourScale, etc.
    auto hgv1 = std::make_unique<mplot::HexGridVisual<FLT>> (RD.hg.get(), spatOff);
    v1.bindmodel (hgv1);
    ...
    hgv1->cm.setType (cmt);
    //hgv1->hexVisMode = mplot::HexVisMode::Triangles;
    hgv1->addLabel ("Variable A", { -0.2f, RD.ellipse_b*-1.4f, 0.01f },
                    mplot::TextFeatures(0.1f, 48, mplot::colour::white));
    ...
    xzero += RD.hg->width();
    spatOff = { xzero, 0.0, 0.0 };
    auto hgv2 = std::make_unique<mplot::HexGridVisual<FLT>> (RD.hg.get(), spatOff);
    v1.bindmodel (hgv2);
    ...
    std::string tnow = mplot::tools::timeNow();
